...
Pesquisas como as de Nguyen-Viet (2025)\cite{NguyenViet2025} e Maratou et al. (2023)\cite{Maratou2023} apontam que elementos como missões, recompensas e feedback visual aumentam a motivação intrínseca dos usuários, reforçando tanto a sensação de competência quanto o senso de progresso. 
...
\section{Público-Alvo e Escopo}

O GenApp destina-se a indivíduos que buscam aprimorar sua saúde financeira, com foco especial em jovens adultos em início de carreira e pessoas com baixa literacia financeira. O público-alvo abrange:
\begin{itemize}
    \item \textbf{Jovens Adultos e Universitários:} Em fase de transição para a independência financeira.
    \item \textbf{Famílias em Estruturação:} Que necessitam organizar o orçamento para alcançar metas de médio e longo prazo.
    \item \textbf{Usuários Motivados por Gamificação:} Que preferem abordagens lúdicas e interativas para o engajamento em tarefas rotineiras.
\end{itemize}

\subsection*{Delimitação do Escopo}
O projeto concentra-se na educação e organização financeira pessoal, atuando como uma ferramenta complementar e não substitutiva aos aplicativos bancários.
\begin{itemize}
    \item \textbf{Foco:} Registro manual de transações para conscientização, cálculo de índices (TPS, RDR) e sistema de missões.
    \item \textbf{Limitações:} A versão atual não contempla integração automática com instituições financeiras (Open Finance), priorizando a segurança e a simplicidade da arquitetura inicial.
\end{itemize}
...
\noindent\textit{Nota de referência} — As definições e interpretações completas dos índices seguem abaixo. Para uma visão de implementação consolidada (fórmulas finais, vínculos de transações e rotinas de processamento), consulte o Capítulo~\ref{chap:impl-consolidada}.
...
\subsection{Taxa de Poupança Pessoal (TPS)}
 A Taxa de Poupança Pessoal (TPS), do inglês *Personal Savings Rate*, mede a porcentagem da renda mensal total que é efetivamente poupada após o pagamento de todas as despesas e obrigações de dívida. Trata-se de um indicador crucial para avaliar a capacidade de um indivíduo em acumular patrimônio, preparar-se para emergências e alcançar objetivos financeiros de longo prazo. Autores como Gitman (2012) \cite{gitman2012} enfatizam a importância da poupança regular como um dos pilares da administração financeira pessoal.
    ...
    extbf{Cálculo:}
$$
    ext{TPS} = \left(\frac{\text{Receitas Totais} - \text{Despesas Totais} - \text{Pagamentos de Despesas}}{\text{Receitas Totais}}\right) \times 100
$$
...
Onde \textit{Receitas Totais} representa todos os rendimentos do período (salários, freelances, rendimentos de investimentos, etc.), \textit{Despesas Totais} são os gastos do dia a dia (alimentação, moradia, transporte, lazer, etc.), e \textit{Pagamentos de Despesas} representam desembolsos classificados como comprometidos ou recorrentes (parcelas de empréstimos, financiamentos e pagamentos mínimos de cartões quando informados). Para efeitos de cálculo interno, o sistema distingue esses lançamentos por meio de vínculos de transação (links) e grupos de categoria.
...
$$
Onde a ``Soma dos Pagamentos Mensais de Todas as Dívidas'' inclui parcelas de empréstimos, financiamentos (imobiliário, veículo), pagamentos mínimos de cartões de crédito, e outras obrigações financeiras regulares. As ``Receitas Totais'' referem-se ao total de rendimentos do período, incluindo salários, freelances, e outras fontes de renda. No contexto do GenApp,\ apenas são considerados os pagamentos explicitamente \textit{vinculados} a dívidas por meio de vínculos de transação.
...
\begin{itemize}
    ...
    \item \textbf{Pagamentos Mensais de Despesas Comprometidas (vinculadas a dívidas)}: R\$ 2.100,00, composto por: 
    \begin{itemize}
...
\]
Isso significa que 42\% das receitas de João estão comprometidas com pagamentos vinculados a dívidas (despesas comprometidas). Este valor está na faixa de atenção (36--42\%), próximo ao limite crítico de 43\%.
...
Neste capítulo, detalhamos o processo de modelagem do GenApp, desde a metodologia e ferramentas utilizadas até a concepção da arquitetura e design.

\section{Metodologia e Ferramentas}
O desenvolvimento seguiu uma abordagem ágil adaptada, utilizando ferramentas colaborativas para gestão e prototipação.

\subsection{Gestão de Projeto}
O \textbf{Trello} foi utilizado para o acompanhamento de tarefas (Kanban), permitindo a visualização clara do progresso e a organização das etapas de desenvolvimento.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\linewidth]{./imagens/trello.png}
    \caption{Quadro de gestão de tarefas no Trello}
    \label{fig:trello}
\end{figure}

\subsection{Prototipação (UI/UX)}
O \textbf{Figma} serviu como plataforma para o design das interfaces e fluxo de navegação. A prototipação de alta fidelidade antecipou decisões de usabilidade, reduzindo o retrabalho durante a implementação.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\linewidth]{./imagens/figma.png}
    \caption{Prototipação de interfaces no Figma}
    \label{fig:figma}
\end{figure}

\subsection{Versionamento de Código}
O \textbf{GitHub} foi utilizado para hospedagem e controle de versão, facilitando a integração contínua e o histórico de alterações do código-fonte.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\linewidth]{./imagens/github.png}
...
\label{chap:impl-consolidada}

Este capítulo detalha o processo de construção do GenApp, descrevendo as tecnologias selecionadas, a arquitetura implementada e os mecanismos de segurança adotados.

\section{Tecnologias Utilizadas}
A seleção tecnológica priorizou ferramentas modernas, robustas e com amplo suporte da comunidade, garantindo produtividade e escalabilidade.

\subsection{Frontend: Flutter}
O desenvolvimento da interface utilizou o \textbf{Flutter}, framework multiplataforma do Google. A escolha se justificou pela capacidade de renderização nativa e pelo ecossistema rico de pacotes.
\begin{itemize}
    \item \textbf{Interface:} Construída com widgets do Material Design, garantindo responsividade e estética moderna.
    \item \textbf{Gráficos:} A biblioteca \texttt{fl\_chart} foi empregada para a visualização dinâmica dos dados financeiros.
    \item \textbf{Comunicação:} O pacote \texttt{dio} gerenciou as requisições HTTP seguras (TLS).
    \item \textbf{Persistência Local:} Uma abordagem híbrida com \textbf{Hive} (chave-valor) e \textbf{Drift} (SQLite) assegurou a funcionalidade \textit{offline-first}.
    \item \textbf{Multiplataforma:} O Flutter foi empregado para garantir a compatibilidade com Android e Web.
\end{itemize}

\subsection{Backend: Python e Django}
O servidor foi implementado em \textbf{Python} com o framework \textbf{Django}, escolhido pela sua arquitetura segura e eficiente ("baterias inclusas").
\begin{itemize}
    \item \textbf{API RESTful:} O \textbf{Django REST Framework (DRF)} estruturou os endpoints, serializadores e validações.
    \item \textbf{Autenticação:} Utilizou-se \texttt{simplejwt} para gestão de tokens JWT.
    \item \textbf{Assincronismo:} O \textbf{Celery}, com \textbf{Redis} como \textit{broker}, gerenciou tarefas em segundo plano (IA e manutenção).
\end{itemize}

\subsection{Banco de Dados: PostgreSQL}
O \textbf{PostgreSQL} foi adotado como Sistema Gerenciador de Banco de Dados (SGBD) devido à sua conformidade ACID, robustez para transações financeiras e suporte a consultas complexas.

\section{Segurança e Privacidade}
A segurança foi um requisito transversal em todo o desenvolvimento, com foco na proteção dos dados sensíveis e conformidade com a LGPD.

\subsection{Autenticação e Criptografia}
\begin{itemize}
    \item \textbf{Senhas:} Armazenadas exclusivamente como hashes (PBKDF2/SHA-256 com salt), impossibilitando a leitura em texto plano.
    \item \textbf{Tokens:} A autenticação via JWT (JSON Web Tokens) elimina a necessidade de sessões no servidor, com tokens armazenados de forma segura no cliente (\texttt{flutter\_secure\_storage}).
    \item \textbf{Transporte:} Toda a comunicação é criptografada via TLS (HTTPS).
\end{itemize}

\subsection{Conformidade com a LGPD}
O sistema adota o princípio da \textit{data minimization}, coletando apenas e-mail e nome para o cadastro. O usuário mantém controle total sobre seus dados, com funcionalidades explícitas para exclusão de conta.

\section{Arquitetura e Processamento}
O backend segue o padrão \textit{Model-Template-View} (MTV) do Django, adaptado ao contexto de API REST, em que os templates são substituídos por serializadores e validações. Uma camada de serviços concentra regras de negócio e cálculos financeiros, favorecendo separação de responsabilidades e testes unitários. Para tarefas assíncronas e de processamento intensivo, utiliza-se \textbf{Celery} com \textbf{Redis} como \textit{message broker}. O sistema adota uma estratégia de cálculo sob demanda com \textit{caching} baseado em tempo de vida (TTL) para os indicadores financeiros (TPS, RDR, ILI). Essa abordagem substitui o modelo tradicional de \textit{snapshots} diários estáticos, garantindo que os dados apresentados ao usuário reflitam o estado mais atual das transações com latência mínima. O Celery é empregado primordialmente para a geração assíncrona de missões personalizadas via IA e para rotinas de manutenção periódica do banco de dados.

\subsection*{Tarefas Assíncronas (resumo)}
\renewcommand{\arraystretch}{1.2}
\begin{table}[H]
    \centering
    \caption{Principais tarefas assíncronas do sistema}
    \label{tab:tasks-assincronas-condensado}
    \begin{tabularx}{\textwidth}{@{}p{4.4cm}p{2.8cm}X@{}}
        \toprule
        \textbf{Tarefa} & \textbf{Agenda} & \textbf{Objetivo} \\
        \midrule
        Geração de Missões (IA) & Sob demanda & Criar missões personalizadas utilizando o Gemini em background para não bloquear a API. \\
        Limpeza de Missões & Periódica & Remover missões antigas ou expiradas para manter o banco otimizado. \\
        Expiração de Cache & Automática (TTL) & Invalidar caches de indicadores e análises após período definido (ex: 5-15 min). \\
        \bottomrule
    \end{tabularx}
\end{table}

\section{Gerenciamento de Estado e Integração Frontend}
O frontend adota uma arquitetura reativa baseada no padrão de gerenciamento de estado nativo do Flutter, otimizada para performance e manutenibilidade.
\begin{itemize}
    \item \textbf{Gerenciamento de Estado:} Utiliza-se a classe \texttt{ChangeNotifier} para encapsular a lógica de estado e regras de negócio da interface. A propagação de mudanças na árvore de widgets é realizada de forma eficiente através de \texttt{InheritedNotifier} (implementado como \texttt{SessionScope}), evitando reconstruções desnecessárias e acoplamento excessivo.
    \item \textbf{Camada de Repositório:} A comunicação com o backend é abstraída pelo padrão \textit{Repository}. O \texttt{AuthRepository} e demais repositórios centralizam as chamadas HTTP via biblioteca \texttt{dio}, tratando serialização JSON, interceptação de erros, renovação automática de tokens JWT e persistência local segura, oferecendo uma API limpa para a camada de apresentação.
\end{itemize}

\section{Inteligência Artificial e Gamificação}
Missões são geradas por modelo generativo (Gemini) a partir de um \textit{prompt} estruturado que considera perfil e métricas do usuário. Cenários temáticos e \textit{tiers} graduam a dificuldade e o impacto esperado, permitindo progressão gradual e feedback motivador. Para controle de custos e robustez, aplicam-se salvaguardas como limitação de frequência de geração, reaproveitamento de sugestões recentes quando adequado e validações de conteúdo antes da publicação ao usuário.

\section{Indicadores e Vinculação de Transações}
Os indicadores refletem o estado financeiro do usuário e guiam recomendações. A terminologia \textit{pagamentos de despesas} é adotada para uniformidade de entendimento.

\begin{equation}\label{eq:tps-condensado}
TPS = \frac{Receitas - Despesas - Pagamentos\;de\;Despesas\;Vinculados}{Receitas} \times 100
\end{equation}

\begin{equation}\label{eq:rdr-condensado}
RDR = \frac{Pagamentos\;de\;Despesas\;Vinculados}{Receitas} \times 100
\end{equation}

\begin{equation}\label{eq:ili-condensado}
ILI = \frac{Despesas\;Essenciais\;Médias\;90d}{Receitas\;Médias\;90d} \times 100
\end{equation}

\noindent\textit{Nota:} As ligações explícitas entre receitas e despesas (\textit{links} de transação) evitam dupla contagem e permitem atribuição de saídas a entradas, afetando diretamente \eqref{eq:tps-condensado} e \eqref{eq:rdr-condensado}.

\section{Segurança e Deploy}
SENHAS são tratadas pelo mecanismo padrão do Django (PBKDF2 com SHA-256, com \textit{salt} e múltiplas iterações). A autenticação utiliza JWT no backend e armazenamento seguro de credenciais no aplicativo cliente. A comunicação deve ocorrer via TLS. O Railway é empregado exclusivamente como ambiente de validação e testes controlados, não caracterizando infraestrutura de produção.

\section{Testes e Próximos Passos}
Há cobertura inicial para serviços de cálculo e validações de serialização. Como próximos passos, recomenda-se ampliar testes de integração (incluindo tarefas assíncronas) e testes de interface no aplicativo.

\begin{table}[H]
    \centering
    \caption{Resumo de testes (estado atual e planejados)}
    \label{tab:testes-condensado}
    \begin{tabularx}{\textwidth}{@{}p{4.6cm}p{3.2cm}X@{}}
        	oprule
        	extbf{Componente} & \textbf{Tipo} & \textbf{Objetivo} \\
        \midrule
        Serviços financeiros & Unitário & Validar cálculos (TPS, RDR, ILI) e invalidação de cache. \\
        Serializers (principais) & Unitário & Garantir estrutura e validação de dados em endpoints críticos. \\
        Tarefas Celery & Integração (planejado) & Verificar execução e agendamento com dados de teste. \\
        Aplicativo (Flutter) & Widget/integração (planejado) & Assegurar fluxo de autenticação e dashboard básico. \\
        \bottomrule
    \end{tabularx}
\end{table}
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|p{3.5cm}|X|p{3.5cm}|}
        ...
        Criação de wireframes e fluxos de navegação; Protótipo visual (Figma); &
        Junho de 2025 \\
        \hline
...
\appendix
\chapter{Apêndices Técnicos}

\section{Prompt de Geração de Missões (IA)}\label{apendice:prompt-missoes}
O prompt estruturado incorpora: (i) perfil e métricas (TPS, RDR, ILI), (ii) distribuição recente de gastos por categoria, (iii) contexto temporal (mensal/diário) e (iv) restrições de escopo e linguagem clara. O objetivo é obter missões específicas, atingíveis e alinhadas a cenário e \textit{tier} selecionados. Abaixo, um excerto representativo:

\begin{verbatim}
Contexto do usuário: métricas {TPS, RDR, ILI}, gastos por categoria, metas ativas.
Tarefa: propor 3 missões concisas, específicas e mensuráveis, com prazo e critério
de validação. Limitar a instruções praticáveis, evitar jargões técnicos.
Restrições: respeitar salvaguardas de custo; evitar recomendações inviáveis.
Saída: JSON com {titulo, descricao, prazo, criterio, tier, categoria foco}.
\end{verbatim}

\section{Tarefas Assíncronas Programadas (Celery/Beat)}\label{apendice:celery-beat}
O sistema agenda rotinas essenciais para a manutenção e inteligência do sistema, substituindo a necessidade de processamentos em lote monolíticos:
\begin{itemize}
    \item \textbf{Geração de Missões (Sob Demanda/Agendada):} Orquestra a criação de novas missões via IA (Gemini) para usuários elegíveis. O processamento ocorre em \textit{background workers}, evitando o bloqueio da API principal e permitindo retentativas automáticas em caso de falha na comunicação com o LLM.
    \item \textbf{Limpeza de Missões (Periódica):} Remove ou arquiva missões expiradas e não concluídas, mantendo a integridade referencial e a performance das consultas ao banco de dados.
    \item \textbf{Gerenciamento de Cache (Automática):} Embora o Redis gerencie a expiração de chaves (TTL) nativamente, rotinas auxiliares garantem a invalidação proativa de indicadores financeiros quando ocorrem transações retroativas, assegurando consistência eventual.
\end{itemize}
As tarefas são executadas por \textit{workers} Celery distribuídos, utilizando Redis como \textit{message broker}, o que garante escalabilidade horizontal e desacoplamento do ciclo de requisição-resposta HTTP.

\section{Salvaguardas de Custo e Uso de IA}\label{apendice:ia-salvaguardas}
Para evitar custos excessivos e garantir estabilidade:
\begin{itemize}
    \item Limitar a frequência de geração por usuário e por período.
    \item Cachear recomendações recentes quando aplicável.
    \item Validar conteúdo e tamanho da resposta antes de persistir.
    \item Degradar graciosamente para missões predefinidas caso haja indisponibilidade.
\end{itemize}
