...
\begin{itemize}\itemsep=5pt
    \item Arquitetura do Django baseada no padrão \textbf{MTV (Model-Template-View)}; no contexto de API utiliza-se as \textbf{Views do DRF} e \textbf{Serializers} para organizar a lógica e os contratos de dados;
    ...
    \item Ecossistema maduro; neste projeto utiliza-se \texttt{djangorestframework-simplejwt} (tokens JWT).
\end{itemize}
...
As senhas não são armazenadas em texto plano. O \textbf{Django} utiliza por padrão \textbf{PBKDF2 com SHA-256}, com \textit{salt} e \textbf{múltiplas iterações}, o que mitiga ataques de dicionário e \textit{rainbow tables}. O framework também oferece suporte a algoritmos como \textbf{Argon2}, \textbf{bcrypt} e \textbf{scrypt}.
...
\begin{enumerate}\itemsep=3pt
    ...
    \item O backend armazena a senha utilizando o mecanismo padrão do Django (\textbf{PBKDF2 com SHA-256}, com \textit{salt} e múltiplas iterações).
\end{enumerate}
...
\chapter{Implementação Técnica Consolidada}
\label{chap:impl-consolidada}

Este capítulo sintetiza, em linguagem objetiva, os principais aspectos técnicos efetivamente implementados no GenApp, preservando a coerência com as seções anteriores e oferecendo uma visão consolidada para referência rápida.

\section{Arquitetura e Processamento}
O backend segue o padrão \textit{Model-Template-View} (MTV) do Django, adaptado ao contexto de API REST, em que os templates são substituídos por serializadores e validações. Uma camada de serviços concentra regras de negócio e cálculos financeiros, favorecendo separação de responsabilidades e testes unitários. Para tarefas de maior custo e processamento periódico, utiliza-se Celery com Redis como corretor, permitindo a execução assíncrona de rotinas de snapshots diários e mensais, além de políticas de expiração/renovação de cache de indicadores, reduzindo recomputações em leituras frequentes.
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{@{}p{4.4cm}p{2.8cm}X@{}}
        ...
        \midrule
        Snapshot diário de usuários & Diária (23:59) & Congelar métricas e agregados para evolução temporal \newline (TPS, RDR, ILI, totais, uso por categoria). \\
        Snapshot diário de missões & Diária (23:59) & Registrar progresso e critérios atingidos das missões ativas. \\
        Snapshot mensal consolidado & Últimos dias do mês & Consolidar indicadores para análises macro mensais. \\
        \bottomrule
    \end{tabularx}
\end{table}
...
O sistema agenda três rotinas principais:
\begin{itemize}
    \item \textbf{Snapshot diário de usuários (23:59):} congela métricas e agregados do dia.
    \item \textbf{Snapshot diário de missões (23:59):} registra progresso e cumprimento de critérios.
    \item \textbf{Snapshot mensal consolidado (últimos dias do mês):} compila visão macro.
\end{itemize}
As tarefas são executadas por workers Celery, com Redis como corretor, e utilizam o fuso \textit{America/Sao\_Paulo}. Resultados não necessários são suprimidos para eficiência.
