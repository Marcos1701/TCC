...
class AdminActionLog(models.Model):
    """
    Modelo para registrar todas as ações administrativas realizadas em usuários.
    
    Usado para auditoria e rastreamento de alterações feitas por admins/staff.
    """
    ...
    def log_action(cls, admin_user, target_user, action_type, old_value=None, new_value=None, reason='', ip_address=None):
        """
        Método auxiliar para criar um log de ação.
        
        Args:
            admin_user: User que realizou a ação
            target_user: User que recebeu a ação
            action_type: Tipo da ação (usar ActionType)
            old_value: Valor anterior (opcional)
            new_value: Novo valor (opcional)
            reason: Motivo da ação (opcional)
            ip_address: IP do admin (opcional)
        
        Returns:
            AdminActionLog: Instância criada
        """
        import json
