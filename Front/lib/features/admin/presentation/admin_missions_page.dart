...
class _AdminMissionsPageState extends State<AdminMissionsPage>
          ...
          children: [
            ...
            if (widget.viewModel.missionsTotalPages > 1)
              _Pagination(
                ...
                totalItems: widget.viewModel.missionsTotalCount,
                onPageChanged: (page) {
                  final isActiveTab = _tabController.index == 0;
                  widget.viewModel.loadMissions(
                    tipo: _filtroTipo,
  ...
  Widget _buildMissionsList({required bool isActiveTab}) {
    ...
    // Quando na aba de pendentes, todas as missões retornadas são pendentes
    // (o backend já filtra por is_active=false), então usamos o total da lista
    final pendingCount = isActiveTab ? 0 : viewModel.missions.length;

    return Column(
      children: [
        // Botões de ação em lote apenas na aba de pendentes
              ...
              children: [
                ...
                Row(
                  children: [
                    // Aprovar Todas
                    ElevatedButton.icon(
                      onPressed: () => _confirmApproveAllPending(),
                      icon: const Icon(Icons.check_circle_outline, size: 18),
                      label: Text('Aprovar Todas ($pendingCount)'),
  ...
  void _confirmDeleteAllPending() {
    // O backend já filtrou por is_active=false, então todas as missões são pendentes
    final pendingCount = widget.viewModel.missions.length;
  ...
  void _confirmApproveAllPending() {
    // O backend já filtrou por is_active=false, então todas as missões são pendentes
    final pendingCount = widget.viewModel.missions.length;
    ...
    showDialog(
      context: context,
      builder: (dialogContext) => AlertDialog(
        title: const Text('Aprovar Todas Pendentes'),
        content: Text(
          'Tem certeza que deseja ativar $pendingCount missão(ões) pendente(s)?\n\n'
          'Todas as missões serão disponibilizadas para os usuários.',
        ),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(dialogContext),
          ...
          ElevatedButton(
            onPressed: () async {
              ...
              final resultado = await widget.viewModel.approveAllPendingMissions();

              if (mounted) {
                final sucesso = resultado['sucesso'] == true;
                final aprovadas = resultado['aprovadas'] ?? pendingCount;
                
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(
                    content: Text(
                      sucesso
                          ? '$aprovadas missão(ões) ativada(s)!'
                          : resultado['erro'] ?? 'Erro ao aprovar missões',
                    ),
                    backgroundColor: sucesso ? Colors.green : Colors.red,
                  ),
                );
                ...
                if (sucesso) {
                  _tabController.animateTo(0); // Vai para aba de ativas
                  _aplicarFiltros();
                }
              }
            },
            ...
            child: const Text('Aprovar Todas'),
          ),
        ],
      ),
    );
        ...
        child: SingleChildScrollView(
          child: Column(
            ...
            children: [
              ...
              // Card de tipo da missão com ícone colorido
              Container(
                padding: const EdgeInsets.all(12),
                decoration: BoxDecoration(
                  color: MissionTypeColors.get(missionType).withValues(alpha: 0.1),
                  borderRadius: BorderRadius.circular(8),
                  border: Border.all(
                    color: MissionTypeColors.get(missionType).withValues(alpha: 0.3),
                  ),
                ),
                child: Row(
                  children: [
                    Icon(
                      MissionTypeIcons.get(missionType),
                      size: 32,
                      color: MissionTypeColors.get(missionType),
                    ),
                    const SizedBox(width: 12),
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          Text(
                            MissionTypeLabels.getDescriptive(missionType),
                            style: theme.textTheme.titleMedium?.copyWith(
                              color: MissionTypeColors.get(missionType),
                              fontWeight: FontWeight.bold,
                            ),
                          ...
                          Text(
                            validationLabels[validationType] ?? validationType,
                            style: theme.textTheme.bodySmall?.copyWith(
                              color: colorScheme.onSurfaceVariant,
                            ),
                          ),
              ...
              if (mission['target_tps'] != null ||
                  ...
                  mission['min_transactions'] != null ||
                  mission['target_reduction_percent'] != null) ...[
                Text(
                  'Metas',
              ...
              // Categoria alvo (se existir)
              if (mission['target_category'] != null) ...[
                _DetailRow(
                  label: 'Categoria Alvo',
                  value: (mission['target_category'] as Map?)?['name']?.toString() ?? 'Categoria ID ${mission['target_category_id']}',
              ...
              // Origem (manual ou sistema)
              _DetailRow(
                label: 'Origem',
                value: (mission['is_system_generated'] as bool? ?? false) 
                    ? 'Sistema (IA/Template)' 
                    : 'Criação Manual',
                icon: (mission['is_system_generated'] as bool? ?? false) 
                    ? Icons.auto_awesome 
                    : Icons.person_outline,
                valueColor: (mission['is_system_generated'] as bool? ?? false) 
              ...
              if (mission['updated_at'] != null)
                _DetailRow(
                  label: 'Atualizada em',
                  value: _formatDate(mission['updated_at'] as String),
                  icon: Icons.update,
                ),
            ],
          ),
